...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; M-x shell の ANSI カラー
;;
;; comint 自体は ANSI カラーをデフォルトで描画できるが、emacs 29 は TERM=dumb
;; でシェルを起動するため ls/git 等が色出力をやめてしまう。dumb-emacs-ansi
;; (色は対応・カーソル移動は非対応と申告する terminfo)を使うと色が出る。
;; emacs 30 ではこれがデフォルトになったので、この設定は 29 専用。
...
あれば選択を求める。"
  ...
  (let* ((root (my/project-root-or-default))
    ...
    (make-process
     ...
     ;; CR(^M)によるプログレスバー再描画を吐き、普通のバッファでは制御文字
     ;; まみれになる。pipe なら非 TTY を検出して行単位のプレーンなログに
     ;; 切り替わる。
     :connection-type 'pipe
     ;; 残った色コードや CR はフィルタで整形する
